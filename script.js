...
function generateNewIntegration() {
    ...
    renderIntegrate();
    setTimeout(() => {
        if(window.showToast) window.showToast('Nova chave gerada com sucesso. Atualize o código no HTML do seu quiz.', 'success');
    }, 100);
}
...
async function renderIntegrate() {
    ...
    const tagSource = `<script \n  async\n  src="https://gestordedados.netlify.app/pixel.js"\n  data-funnel-id="${funnel_id}"\n  data-public-key="${public_key}"\n  data-endpoint="https://gestordedados.netlify.app/api/track">\n</script>`;
...
&lt;a href="#" data-track="checkout-click"&gt;Comprar&lt;/a&gt;
    ...
    window.testIntegration = async function() {
        ...
        try {
            const req = await fetch('https://gestordedados.netlify.app/api/track', {
                method: 'POST',
